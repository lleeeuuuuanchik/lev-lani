<script setup>
	/**
	 * Главная страница. SEO: title, description, Open Graph, Twitter Card, canonical.
	 * JSON-LD: BeautySalon + WebSite + ItemList услуг + AggregateRating из отзывов.
	 */

	// variables
	const SITE_URL = 'https://levlani.ru';
	const { servicesApi, reviewsApi } = useApi();
	const { data: services } = await servicesApi.getServices();
	const { data: reviews } = await reviewsApi.getReviews();
